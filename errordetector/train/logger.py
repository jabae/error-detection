...
class Logger(object):
  ...
  def log_images(self, phase, iter_num, preds, sample):
    ...
    # Log additional resource
    tag = '{}/images/{}'.format(phase, 'machine_label')
    tensor = sample["obj_mask"][0,...]
    self.log_image(tag, tensor, iter_num)
    ...
    for k  in sorted(self.out_spec):
      if k == 'error1':
        ...
        self.log_image(tag, tensor, iter_num)
        tensor = sample[k][0,...]
        self.log_image('{}/images/{}'.format(phase, 'target1'), tensor, iter_num)
        ...
        self.log_image(tag, tensor, iter_num)
        tensor = sample[k][0,...]
        self.log_image('{}/images/{}'.format(phase, 'target2'), tensor, iter_num)
        ...
        self.log_image(tag, tensor, iter_num)
        tensor = sample[k][0,...]
        self.log_image('{}/images/{}'.format(phase, 'human_label'), tensor, iter_num)
